...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    if(op == A_ADD){
        ovf = (((int)argA<0 && (int)argB<0 && (int)val>0) || 
            ((int)argA>0 && (int)argB>0 && (int)val<0));
    }else if(op == A_SUB) 
    {   
        ovf = (((int)argA<0 && (int)argB>0 && (int)val<0)||
            ((int)argA>0 && (int)argB<0 && (int)val>0));
...
stat_t nexti(y64sim_t *sim)
{
    ...
    long_t addr;
    regid_t raid, rbid;
    long_t rA_val, rB_val, rA_mval, rB_mval, rE_val;
    ...
    switch(icode)
    {
        case I_HALT: case I_NOP: case I_JMP: case I_CALL: case I_RET:
        {
            break;
        }
        default:
        {
            ...
            raid = GET_REGA(codefun);
            rbid = GET_REGB(codefun);
            next_pc++;
            break;
        }
    }
    ...
    switch (icode) {
      ...
      case I_RRMOVQ:  /* 2:x regA:regB */
        ...
        {
            rA_val = get_reg_val(sim->r, raid);
            set_reg_val(sim->r, rbid, rA_val);
        }
    	break;

      case I_IRMOVQ: /* 3:0 imm:regB */
        set_reg_val(sim->r, rbid, addr);
        break;

      case I_RMMOVQ: /* 4:0 regA:regB imm */
        rA_val = get_reg_val(sim->r, raid);
        rB_val = get_reg_val(sim->r, rbid);
        rE_val = compute_alu(A_ADD, rB_val, addr);
        if(!set_long_val(sim->m, rE_val, rA_val))
        {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, rE_val);
            return STAT_ADR;
        }
        break;
      ...
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        rB_val = get_reg_val(sim->r, rbid);
        rE_val = compute_alu(A_ADD, rB_val, addr);
        if(!get_long_val(sim->m, rE_val, &rB_mval))
        {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, rE_val);
            return STAT_ADR;
        }
        set_reg_val(sim->r, raid, rB_mval);
    	break;
      ...
      case I_ALU: /* 6:x regA:regB */
        ...
        rA_val = get_reg_val(sim->r, raid);
        rB_val = get_reg_val(sim->r, rbid);
    	rE_val = compute_alu(ifun, rA_val, rB_val);
        sim->cc = compute_cc(ifun, rA_val, rB_val, rE_val);
        set_reg_val(sim->r, rbid, rE_val);
        break;
      ...
      case I_JMP: /* 7:x imm */
        ...
        next_pc = cond_doit(sim->cc, (cond_t)ifun)? addr: next_pc;
        break;

      case I_CALL: /* 8:x imm */
        rsp = get_reg_val(sim->r, REG_RSP);
        rE_val = compute_alu(A_SUB, 8, rsp);
        set_reg_val(sim->r, REG_RSP, rE_val);
        if (!set_long_val(sim->m, rE_val, next_pc))
        {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rE_val);
            return STAT_ADR;
        }
        next_pc = addr;
        break;
      ...
      case I_RET: /* 9:0 */
        rsp = get_reg_val(sim->r, REG_RSP);
        if(!get_long_val(sim->m, rsp, &next_pc))
        {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rsp);
            return STAT_ADR;
        }
        rE_val = compute_alu(A_ADD, 8, rsp);
        set_reg_val(sim->r, REG_RSP, rE_val);
        break;
      ...
      case I_POPQ: /* B:0 regA:F */
        rsp = get_reg_val(sim->r, REG_RSP);
        if(!get_long_val(sim->m, rsp, &rsp_mval))
        {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rsp);
            return STAT_ADR;
        }
        rE_val = compute_alu(A_ADD, 8, rsp);
    	set_reg_val(sim->r, REG_RSP, rE_val);
        set_reg_val(sim->r, raid, rsp_mval);
        break;
